...
if __name__ == '__main__':
    ...
    if transition_type == 0:
        ...
        prob_remain = [pr_ss_0, pr_sr_0, pr_sp_0, pr_rr_0, pr_rp_0, pr_pp_0, pr_ss_1, pr_sr_1, pr_sp_1, pr_rr_1,
                       pr_rp_1, pr_pp_1]
    ...
    fraction = 0.5
    n_choices = max(1, int(round(fraction * n_arms)))
    ...
    W = WhittleDisT(n_states, n_arms, reward_bandits, transition_bandits)
    W.get_whittle_indices(computation_type=method, params=[0, max_wi], n_trials=n_trials_neutrl)
    w_bandits = W.w_indices

    SafeW = SafeWhittleDisT([n_states, n_parts], n_arms, reward_bandits, transition_bandits, n_steps, u_type, u_order,
                             thresholds)
    SafeW.get_whittle_indices(computation_type=method, params=[0, max_wi], n_trials=n_trials_safety)
    sw_bandits = SafeW.w_indices

    # print('Process Begins ...')
    rew_r, obj_r, _ = ProcessDis_Random(n_episodes, n_steps, n_states, n_arms, n_choices, thresholds, reward_bandits,
                                        transition_bandits, initial_states, u_type, u_order)
    rew_m, obj_m, _ = ProcessDis_Greedy(n_episodes, n_steps, n_states, n_arms, n_choices, thresholds, reward_bandits,
                                        transition_bandits, initial_states, u_type, u_order)
    rew_w, obj_w, _ = ProcessDisT_WhtlRB(W, w_bandits, n_episodes, n_steps, n_states, n_arms, n_choices, thresholds,
                                         reward_bandits, transition_bandits, initial_states, u_type, u_order)
    rew_s, obj_s, _ = ProcessDisT_SafeRB(SafeW, sw_bandits, n_episodes, n_steps, n_states, n_arms, n_choices, thresholds,
                                         reward_bandits, transition_bandits, initial_states, u_type, u_order)
    print('Process Ends ...')
    ...
    if rb_type == 'hard':
        n_episodes = 100
        probs_l, sumwis_l, rew_l, obj_l = ProcessDisT_SingleSafeTSRB(n_iterations, l_episodes, n_episodes, n_steps, n_states, fixed_wi, n_choices, thresholds,
