...
def run_combination(params):
    ...
    WhtlW = WhittleDis(df, ns, na, R.vals, M.transitions, nt)
    WhtlW.get_whittle_indices(computation_type=method, params=[0, int(nt/(1-df))], n_trials=10*int(nt/(1-df)))

    SafeW = SafeWhittleDis(df, [ns, np, nz], na, R.vals, M.transitions, nt, ut, uo, th * numpy.ones(na))
...
def main():
    ...
    PATH1 = f'./output-finite-dis/Res_{param_sets["t_type_set"]}{param_sets["n_states_set"]}{param_sets["armcoef_set"]}.xlsx'
    PATH2 = f'./output-finite-dis/ResAvg_{param_sets["t_type_set"]}{param_sets["n_states_set"]}{param_sets["armcoef_set"]}.xlsx'
    PATH3 = f'./output-finite-dis/'
