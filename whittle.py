...
class SafeWhittleAvg:
    ...
    def __init__(self, num_states, num_arms: int, rewards, transition, horizon, u_type, u_order, thresholds):
            ...
            for x in range(self.num_x):
                ...
                for l in range(self.n_realize[arm][t]):
                    for act in range(2):
                        ...
                        nxt_l = self.get_reward_partition(self.all_total_rewards[l] + imm_reward)
                        # nxt_l = max(0, min(self.n_augment[arm] - 1, l + x))

                        Q[l, x, t, act] = np.round(- penalty * act
                ...
                for act in range(2):
                    if len(self.reward.shape) == 3:
                        Q[x, act] = self.reward[x, act, arm] - penalty * act \
                                    + np.dot(V, self.transition[x, :, act, arm])
                    else:
...
class SafeWhittleOldAvg:
    ...
    def bellman_equation(self, arm, penalty):
        ...
        while diff > 1e-4 and iteration < 1000:
            V_prev = np.copy(V)
            for x in range(self.num_x):
                for l in range(self.n_augment[arm]-1):
                    # print('='*20)
                    # print(x)
                    # print(l)
                    for act in range(2):
