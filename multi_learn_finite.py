...
def run_combination(params):
    nt, ns, np, nc, ft, tt, ut, uo, th, fr, method, l_episodes, n_episodes, n_iterations, PATH3 = params
    na = nc * ns
    ...
    probs_l, _, _, obj_l, _, _, obj_s = Process_LearnSafeTSRB(
        n_iterations, l_episodes, n_episodes, nt, ns, na, nch,
        thresh, tt, True, method, R.vals, M.transitions,
        initial_states, ut, uo, False, nt
    )

    key_value = f'nt{nt}_np{np}_ns{ns}_nc{nc}_ft{ft}_tt{tt}_ut{ut}_uo{uo}_th{th}_fr{fr}'
...
def main():
    ...
    param_list = [
        (nt, ns, np, nc, ft_type, tt, ut, uo, th, fr, method, l_episodes, n_episodes, n_iterations, PATH3)
