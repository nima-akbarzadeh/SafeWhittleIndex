...
def Process_SafeTSRB(n_iterations, n_episodes, n_steps, n_states, n_arms, n_choices, thresholds, t_type, t_increasing, method, tru_rew, tru_dyn, initial_states, u_type):
    ...
    for i in range(n_iterations):
        ...
        SafeW = SafeWhittle(n_states, n_arms, tru_rew, M.transitions, n_steps, u_type, thresholds)
        ...
        for k in range(n_episodes):
            ...
            for a in range(n_arms):
                ...
                if t_type == 5:
                    cnt = []
                    cnt.append(est_transitions[0, -1, 1, a])
                ...
                if t_type == 3:
                    cnt = []
                    cnt.append((1/(n_states-1)) * est_transitions[0, 0, 1, a])
            ...
            SafeW = SafeWhittle(n_states, n_arms, tru_rew, dynamics, n_steps, u_type, thresholds)
            SafeW.get_whittle_indices(computation_type=method, params=[0, 1], n_trials=n_trials_safety)
            sw_indices = SafeW.w_indices
