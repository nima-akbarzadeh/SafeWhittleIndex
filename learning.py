...
def ProcessD_LearnSafeTSRB(n_iterations, l_episodes, n_episodes, n_steps, n_states, n_parts, n_arms, n_choices, thresholds,
    ...
    PlanW = SafeWhittleD([n_states, n_parts], n_arms, tru_rew, tru_dyn, n_steps, u_type, u_order, thresholds)
    PlanW.get_whittle_indices(computation_type=method, params=[0, max_wi], n_trials=n_trials_safety)
    plan_indices = PlanW.w_indices
    plan_sumwis = [np.sum(plan_indices[a]) for a in range(n_arms)]
    ...
    for n in range(n_iterations):
        ...
        if t_type < 10:
            all_learn_probs[n, 0, :] = np.array([np.round(random.uniform(0.1 / n_states, 1 / n_states), 2)
                                                 for _ in range(n_arms)])
            Mest = MarkovDynamics(n_arms, n_states, all_learn_probs[n, 0, :], t_type, t_increasing)
            LearnW = SafeWhittleD([n_states, n_parts], n_arms, tru_rew, Mest.transitions, n_steps, u_type, u_order, thresholds)
        else:
            est_transitions = np.zeros((n_states, n_states, 2, n_arms))
            for a in range(n_arms):
                for s1 in range(n_states):
                    for act in range(2):
                        est_transitions[s1, :, act, a] = dirichlet.rvs(np.ones(n_states))
            LearnW = SafeWhittleD([n_states, n_parts], n_arms, tru_rew, est_transitions, n_steps, u_type, u_order, thresholds)
        ...
        for l in range(l_episodes):
            ...
            if t_type < 10:
                Mest = MarkovDynamics(n_arms, n_states, np.round(all_learn_probs[n, l, :], 2), t_type, t_increasing)
                SafeW = SafeWhittleD([n_states, n_parts], n_arms, tru_rew, Mest.transitions, n_steps, u_type, u_order, thresholds)
                SafeW.get_whittle_indices(computation_type=method, params=[0, max_wi], n_trials=n_trials_safety)
                sw_indices = SafeW.w_indices
            else:
                SafeW = SafeWhittleD([n_states, n_parts], n_arms, tru_rew, est_transitions, n_steps, u_type, u_order, thresholds)
                SafeW.get_whittle_indices(computation_type=method, params=[0, max_wi], n_trials=n_trials_safety)
                sw_indices = SafeW.w_indices
    ...
    if save_data:
        joblib.dump([all_learn_sumwis, all_learn_rewards, all_learn_objectives, plan_sumwis, all_plan_rewards,
                     all_plan_objectives],
                    f'./output-learn-d-finite/safetsrb_{n_episodes}{n_steps}{n_states}{t_type}{u_type}{n_choices}{thresholds[0]}{u_type}{u_order}.joblib')
...
def ProcessNS_LearnSafeTSRB(n_iterations, l_episodes, n_episodes, n_steps, n_states, n_arms, n_choices, thresholds,
    ...
    if save_data:
        joblib.dump([all_learn_sumwis, all_learn_rewards, all_learn_objectives, plan_sumwis, all_plan_rewards,
                     all_plan_objectives],
                    f'./output-learn-ns-finite/safetsrb_{n_episodes}{n_steps}{n_states}{t_type}{u_type}{n_choices}{thresholds[0]}{u_type}{u_order}.joblib')
...
def ProcessDNS_LearnSafeTSRB(n_iterations, l_episodes, n_episodes, n_steps, n_states, n_parts, n_arms, n_choices, thresholds,
    ...
    if save_data:
        joblib.dump([all_learn_sumwis, all_learn_rewards, all_learn_objectives, plan_sumwis, all_plan_rewards,
                     all_plan_objectives],
                    f'./output-learn-dns-finite/safetsrb_{n_episodes}{n_steps}{n_states}{t_type}{u_type}{n_choices}{thresholds[0]}{u_type}{u_order}.joblib')
...
def ProcessNSR_LearnSafeTSRB(n_iterations, l_episodes, n_episodes, n_steps, n_states, n_arms, n_choices, thresholds,
    ...
    if save_data:
        joblib.dump([all_learn_sumwis, all_learn_rewards, all_learn_objectives, plan_sumwis, all_plan_rewards,
                     all_plan_objectives],
                    f'./output-learn-nsr-finite/safetsrb_{n_episodes}{n_steps}{n_states}{t_type}{u_type}{n_choices}{thresholds[0]}{u_type}{u_order}.joblib')
...
def ProcessDNSR_LearnSafeTSRB(n_iterations, l_episodes, n_episodes, n_steps, n_states, n_parts, n_arms, n_choices, thresholds,
    ...
    if save_data:
        joblib.dump([all_learn_sumwis, all_learn_rewards, all_learn_objectives, plan_sumwis, all_plan_rewards,
                     all_plan_objectives],
                    f'./output-learn-dnsr-finite/safetsrb_{n_episodes}{n_steps}{n_states}{t_type}{u_type}{n_choices}{thresholds[0]}{u_type}{u_order}.joblib')

    return all_learn_sumwis, all_learn_rewards, all_learn_objectives, plan_sumwis, all_plan_rewards, all_plan_objectives


def ProcessDis_LearnSafeTSRB(beta, n_iterations, l_episodes, n_episodes, n_steps, n_states, n_parts_s, n_parts_z, n_arms, n_choices,
    ...
    PlanW = SafeWhittleDis(beta, [n_states, n_parts_s, n_parts_z], n_arms, tru_rew, tru_dyn, n_steps, u_type, u_order, thresholds)
    ...
    for n in range(n_iterations):
        ...
        if t_type < 10:
            all_learn_probs[n, 0, :] = np.array([np.round(random.uniform(0.1 / n_states, 1 / n_states), 2)
                                                 for _ in range(n_arms)])
            Mest = MarkovDynamics(n_arms, n_states, all_learn_probs[n, 0, :], t_type, t_increasing)
            LearnW = SafeWhittleDis(beta, [n_states, n_parts_s, n_parts_z], n_arms, tru_rew, Mest.transitions, n_steps, u_type, u_order,
                                    thresholds)
        else:
            est_transitions = np.zeros((n_states, n_states, 2, n_arms))
            for a in range(n_arms):
                for s1 in range(n_states):
                    for act in range(2):
                        est_transitions[s1, :, act, a] = dirichlet.rvs(np.ones(n_states))
            LearnW = SafeWhittleDis(beta, [n_states, n_parts_s, n_parts_z], n_arms, tru_rew, est_transitions, n_steps, u_type, u_order,
                                    thresholds)
        ...
        for l in range(l_episodes):
            ...
            plan_totalrewards, plan_objectives, learn_totalrewards, learn_objectives, cnts = \
                ProcessDis_LearnSafeRB(PlanW, LearnW, beta, n_episodes, n_steps, n_states,
            ...
            if t_type < 10:
                Mest = MarkovDynamics(n_arms, n_states, np.round(all_learn_probs[n, l, :], 2), t_type, t_increasing)
                SafeW = SafeWhittleDis(beta, [n_states, n_parts_s, n_parts_z], n_arms, tru_rew, Mest.transitions, n_steps, u_type, u_order,
                                       thresholds)
                SafeW.get_whittle_indices(computation_type=method, params=[0, 10], n_trials=100)
                sw_indices = SafeW.w_indices
            else:
                SafeW = SafeWhittleDis(beta, [n_states, n_parts_s, n_parts_z], n_arms, tru_rew, est_transitions, n_steps, u_type, u_order,
                                       thresholds)
                SafeW.get_whittle_indices(computation_type=method, params=[0, 10], n_trials=100)
                sw_indices = SafeW.w_indices
...
def ProcessDisInf_LearnSafeTSRB(beta, n_iterations, l_episodes, n_episodes, n_steps, n_states, n_parts_s, n_parts_z, n_arms, n_choices,
                                thresholds, t_type, t_increasing, method, tru_rew, tru_dyn, initial_states, u_type, u_order, save_data):
    ...
    PlanW = SafeWhittleDisInf(beta, [n_states, n_parts_s, n_parts_z], n_arms, tru_rew, tru_dyn, n_steps, u_type, u_order, thresholds)
    PlanW.get_whittle_indices(computation_type=method, params=[0, 10], n_trials=100)
    plan_indices = PlanW.w_indices
    plan_sumwis = [np.sum(plan_indices[a]) for a in range(n_arms)]
    ...
    for n in range(n_iterations):
        ...
        if t_type < 10:
            all_learn_probs[n, 0, :] = np.array([np.round(random.uniform(0.1 / n_states, 1 / n_states), 2)
                                                 for _ in range(n_arms)])
            Mest = MarkovDynamics(n_arms, n_states, all_learn_probs[n, 0, :], t_type, t_increasing)
            LearnW = SafeWhittleDisInf(beta, [n_states, n_parts_s, n_parts_z], n_arms, tru_rew, Mest.transitions, n_steps, u_type, u_order, thresholds)
        else:
            est_transitions = np.zeros((n_states, n_states, 2, n_arms))
            for a in range(n_arms):
                for s1 in range(n_states):
                    for act in range(2):
                        est_transitions[s1, :, act, a] = dirichlet.rvs(np.ones(n_states))
            LearnW = SafeWhittleDisInf(beta, [n_states, n_parts_s, n_parts_z], n_arms, tru_rew, est_transitions, n_steps, u_type, u_order, thresholds)
        ...
        for l in range(l_episodes):
            ...
            plan_totalrewards, plan_objectives, learn_totalrewards, learn_objectives, cnts = \
                ProcessDisInf_LearnSafeRB(PlanW, LearnW, beta, n_episodes, n_steps, n_states, n_arms, n_choices, thresholds, tru_rew, tru_dyn, initial_states, u_type, u_order)
            counts = counts + cnts
            ...
            if t_type < 10:
                Mest = MarkovDynamics(n_arms, n_states, np.round(all_learn_probs[n, l, :], 2), t_type, t_increasing)
                SafeW = SafeWhittleDisInf(beta, [n_states, n_parts_s, n_parts_z], n_arms, tru_rew, Mest.transitions, n_steps, u_type, u_order, thresholds)
                SafeW.get_whittle_indices(computation_type=method, params=[0, 10], n_trials=100)
                sw_indices = SafeW.w_indices
            else:
                SafeW = SafeWhittleDisInf(beta, [n_states, n_parts_s, n_parts_z], n_arms, tru_rew, est_transitions, n_steps, u_type, u_order, thresholds)
                SafeW.get_whittle_indices(computation_type=method, params=[0, 10], n_trials=100)
                sw_indices = SafeW.w_indices
    ...
    if save_data:
        joblib.dump([all_learn_sumwis, all_learn_rewards, all_learn_objectives, plan_sumwis, all_plan_rewards,
                     all_plan_objectives],
                    f'./output-learn-disinf/safetsrb_{beta}{n_episodes}{n_steps}{n_states}{t_type}{u_type}{n_choices}{thresholds[0]}{u_type}{u_order}.joblib')
