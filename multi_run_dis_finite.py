...
def run_combination(params):
    ...
    WhtlW = WhittleDisT(df, ns, na, R.vals, M.transitions, nt)
    WhtlW.get_whittle_indices(computation_type=method, params=[0, int(nt/(1-df))], n_trials=10*int(nt/(1-df)))

    SafeW = SafeWhittleDisT(df, [ns, np], na, R.vals, M.transitions, nt, ut, uo, th * numpy.ones(na))
    ...
    processes = [
        ...
        ("Whittl", lambda *args: ProcessDisT_WhtlRB(WhtlW, WhtlW.w_indices, *args)),
        ("Safaty", lambda *args: ProcessDisT_SafeRB(SafeW, SafeW.w_indices, *args))
    ]
...
def main():
    ...
    param_sets = {
        ...
        'u_type_set': [1, 2], # 1
        'u_order_set': [4, 16], # 16
        'threshold_set': [0.1, 0.2, 0.3], # 0.3
        'fraction_set': [0.3, 0.5], # 0.3
        'discount_set': [0.9, 0.95] # 0.95
    }

    PATH1 = f'./output-dis-finite/Res_{param_sets["t_type_set"]}{param_sets["n_states_set"]}{param_sets["armcoef_set"]}.xlsx'
    PATH2 = f'./output-dis-finite/ResAvg_{param_sets["t_type_set"]}{param_sets["n_states_set"]}{param_sets["armcoef_set"]}.xlsx'
    PATH3 = f'./output-dis-finite/'
    ...
    with Pool(num_cpus) as pool:
        ...
        for count, result in enumerate(pool.imap_unordered(run_combination, param_list), 1):
            ...
            print(f"{count} / {total}: {key_value} ---> AVG-W: {wavg}, AVG-S: {savg}, Diff-SW: {impr_vl}, MEAN-Rel-W: {impr_sw}, MEAN-Rel-M: {impr_sm}, MEAN-Rel-R: {impr_sr}")
