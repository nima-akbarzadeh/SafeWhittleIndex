...
if __name__ == '__main__':
    ...
    count = 0
    total = len(n_steps_set) * len(n_armscoef_set) * len(n_states_set) * len(f_type_set) * len(t_type_set) * len(u_type_set) * len(u_type_set) * len(fraction_set) * len(threshold_set)
                                ...
                                for th in threshold_set:
                                    ...
                                    for fr in fraction_set:
                                        ...
                                        probs_l, sumwis_l, rew_l, obj_l, swi_s, rew_s, obj_s = Process_LearnSoftSafeTSRB(n_iterations, l_episodes, n_episodes, nt, ns, na, nch, thresh, tt,
                                                                                                                         True, method, R.vals, M.transitions, initial_states, ut, uo, True, max_wi)
